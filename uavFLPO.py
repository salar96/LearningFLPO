...
class FLPO():
    ...
    def __init__(self, start_loc:np.ndarray, end_loc:np.ndarray, num_facilities:int, scale:float, disType:str, selfHop:bool):

        self.n = 1
        self.f = num_facilities
        self.startLocation = start_loc
        self.destinationLocation = end_loc
        self.scale = scale
    ...
    def plotFLPO(self, facilityLocations):
        plt.figure(figsize=(4,3))
        plt.scatter(self.startLocation[:,0],self.startLocation[:,1],marker='o', alpha=0.2, label='nodes: '+str(self.n))
        # for facs in facilityLocations:
        #     plt.scatter(facs[:,0],facs[:,1],marker='v')
        plt.scatter(facilityLocations[:,0], facilityLocations[:,1], marker='v', label='facilities: '+ str(self.f))
    ...
    def returnStagewiseCost(self, facs):
        ...
        t1 = time.time()        
        dist_n2fd = cdist(self.startLocation, np.concatenate((facs, self.destinationLocation)), self.distance)
        t2 = time.time()
    ...
    def returnStagewiseGrad(self, facs):
        ...
        diff_n2f = self.startLocation[:,None] - facs[None]
