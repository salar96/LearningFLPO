...
def SolveMIP(s_data, e_data, num_agents, num_nodes, T, dim, y_init=None, gap=0.05):
    ...
    if y_init is not None:
        ...
        y_init_dict = {
            (j, d): 0.5 for j in range(num_nodes) for d in range(dim)
        }
    # === Model ===
    eta_init = {}
    ...
    for a in range(num_agents):
        for t in range(1, T + 1):
            for j in range(num_nodes + 1):
                eta_init[(a, t, j)] = 1 if j == num_nodes else 0
    ...
    # === Variables ===
    model.y = pyo.Var(model.N, range(dim), domain=pyo.Reals, bounds=(0.05, 0.95), initialize=y_init_dict)
    model.eta = pyo.Var(model.A, model.T, model.J, domain=pyo.Binary, initialize=eta_init)
    # Auxiliary variables for SOC form
    model.d2 = pyo.Var(model.A, model.T, domain=pyo.NonNegativeReals)
    ...
    # End stickiness: once at END, must stay at END
    ...
    # === Symmetry-breaking: lexicographic on (x,y) ===
    model.sym_idx = pyo.RangeSet(0, num_nodes - 2)
    # 1) sort by x
    model.sym_x = pyo.Constraint(
        model.sym_idx, rule=lambda m, j: m.y[j, 0] <= m.y[j + 1, 0]
    )
    # 2) tie-break y when x equal (big-M = 1.0 since bounds ∈ [0.05,0.95])
    model.sym_lex = pyo.Constraint(
        model.sym_idx,
        rule=lambda m, j: m.y[j, 1]
        <= m.y[j + 1, 1] + 1.0 * (m.y[j + 1, 0] - m.y[j, 0]),
    )
    ...
    def soc_rule(m, a, t):
        return m.d2[a, t] >= m.xdiff[a, t, 0] ** 2 + m.xdiff[a, t, 1] ** 2

    model.soc = pyo.Constraint(model.A, model.T, rule=soc_rule)

    # === Objective ===
    def total_cost(m):
        return sum(m.d2[a, t] for a in m.A for t in m.T) / num_agents

    model.obj = pyo.Objective(rule=total_cost, sense=pyo.minimize)
    start_time = time.time()
    # === Solve ===
    solver = pyo.SolverFactory("gurobi")
    solver.options["NonConvex"] = 2  # allow convex quadratic constraints with binaries
    solver.options["MIPGap"] = gap  #  optimality gap
...
if __name__ == "__main__":
    ...
    SolveMIP(s_data, e_data, num_agents, num_nodes, T, dim)
