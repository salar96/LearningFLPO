...
def torchFLPO_2_numpyFLPO(START_locs, END_locs, F_base, dim_, scale):
    ...
    start_locs_np = START_locs.cpu().numpy().reshape(-1,dim_)
    end_locs_np = END_locs.cpu().numpy().reshape(-1,dim_)
    f_locs_np = F_base.detach().cpu().numpy().squeeze()

    return start_locs_np, end_locs_np, f_locs_np
